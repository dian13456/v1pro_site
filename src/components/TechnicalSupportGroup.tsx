    ...
    ? createPortal(
        ...
        >
          <div className="mb-2 flex items-start justify-between gap-3 px-1">
            <div>
              <p className="text-sm font-bold text-slate-900 dark:text-white">{t("佳点 V1Pro 售后支持群")}</p>
              <p className="mt-0.5 text-xs text-slate-500 dark:text-slate-400">{t("群号：")}<span className="select-all font-semibold tabular-nums">{SUPPORT_QQ_GROUP}</span>
              </p>
            </div>
            <span className="rounded-full bg-cyan-50 px-2 py-1 text-[10px] font-semibold text-cyan-700 dark:bg-cyan-400/10 dark:text-cyan-200">{t("技术支持")}</span>
          </div>
          <img
            src="/support/qq-group-1029622084.jpg"
            alt={t("技术支持QQ群 {0} 二维码", undefined, {0: SUPPORT_QQ_GROUP})}
            className="aspect-square w-full rounded-xl border border-slate-100 bg-white object-cover object-center"
            loading="lazy"
          />
          <p className="mt-2 text-center text-xs text-slate-500 dark:text-slate-400">{t("使用 QQ 扫码加入群聊")}</p>
        </div>,
        document.body
      )
    : null;
  ...
  return (
    <>
      <button
        ...
        aria-label={t("查看技术支持QQ群 {0}", undefined, {0: SUPPORT_QQ_GROUP})}
        aria-expanded={open}
        aria-controls={popupId}
      >
        <span aria-hidden="true" className="text-base leading-none">QQ</span>
        <span className={`hidden whitespace-nowrap ${compact ? "2xl:inline" : "sm:inline"}`}>{t("技术支持QQ群")}</span>
