import { useI18n, translate as t, LanguageSwitcher } from "../i18n";
...
export function ResourceLibraryHeader({
  keyword,
  onSearch,
  searchPlaceholder = t("搜索素材，如「孤独摇滚」「眨眼」…"),
  ...
  return (
    ...
    <header ref={headerRef} className="resource-library-header sticky top-0 z-[80] min-h-[64px] border-b border-[#e6e9f2] bg-white dark:border-slate-800 dark:bg-slate-950">
      <div className="flex min-w-0 min-h-[64px] items-center gap-2 px-3 sm:px-4 xl:px-6 max-[767px]:flex-wrap max-[479px]:gap-1.5 max-[479px]:py-2">
        <Link to="/" className="flex shrink-0 items-center gap-2.5 text-xl font-extrabold tracking-[-0.025em] text-[#2b3245] dark:text-white">
          ...
          <span className="hidden 2xl:inline">{t("佳点电子素材库")}</span>
        </Link>
        ...
        >
          <ThemeIcon name="upload" size={15} /> <span className="hidden sm:inline">{t("分享素材")}</span>
        </Link>
        <Link to="/activities" className="hidden h-10 shrink-0 items-center gap-1.5 rounded-full border border-black/[.06] bg-white/70 px-4 text-[13px] font-semibold text-[#4a5270] transition hover:bg-white hover:text-[#0071e3] dark:border-white/10 dark:bg-white/[.06] dark:text-slate-200 2xl:inline-flex">
          <ThemeIcon name="activity" size={15} /> {" "}{t("活动中心")}{" "}</Link>
        <Link to="/webusb-test" className="hidden h-10 shrink-0 items-center gap-1.5 rounded-full border border-black/[.06] bg-white/70 px-4 text-[13px] font-semibold text-[#4a5270] transition hover:bg-white hover:text-[#0071e3] dark:border-white/10 dark:bg-white/[.06] dark:text-slate-200 xl:inline-flex">
          <ThemeIcon name="device" size={15} /> {" "}{t("设备控制")}{" "}</Link>
        <Link to="/profile" className="hidden h-10 shrink-0 items-center gap-1.5 rounded-full border border-black/[.06] bg-white/70 px-4 text-[13px] font-semibold text-[#4a5270] transition hover:bg-white hover:text-[#0071e3] dark:border-white/10 dark:bg-white/[.06] dark:text-slate-200 2xl:inline-flex">
          <ThemeIcon name="user" size={15} /> {" "}{t("个人中心")}{" "}</Link>
        ...
        >
          <summary className="grid h-10 min-w-10 cursor-pointer list-none place-items-center rounded-full border border-black/[.06] bg-white/75 px-3 text-[13px] font-semibold text-[#4a5270] transition hover:bg-white hover:text-[#0071e3] dark:border-white/10 dark:bg-white/[.06] dark:text-slate-200 [&::-webkit-details-marker]:hidden">
            <span aria-hidden="true">•••</span><span className="sr-only">{t("更多功能")}</span>
          </summary>
          <div className="absolute right-0 top-[calc(100%+10px)] z-[120] w-48 rounded-[18px] border border-black/[.07] bg-white/95 p-2 shadow-[0_20px_55px_rgba(15,23,42,.18)] backdrop-blur-2xl dark:border-white/10 dark:bg-slate-900/95">
            <div className="grid gap-1 xl:hidden">
              <Link to="/webusb-test" onClick={() => setActiveMenu(null)} className="rounded-xl px-3 py-2 text-sm text-slate-600 hover:bg-slate-100 dark:text-slate-200 dark:hover:bg-white/[.07]">{t("设备控制")}</Link>
            </div>
            <div className="grid gap-1 2xl:hidden">
              <Link to="/activities" onClick={() => setActiveMenu(null)} className="rounded-xl px-3 py-2 text-sm text-slate-600 hover:bg-slate-100 dark:text-slate-200 dark:hover:bg-white/[.07]">{t("活动中心")}</Link>
              <Link to="/profile" onClick={() => setActiveMenu(null)} className="rounded-xl px-3 py-2 text-sm text-slate-600 hover:bg-slate-100 dark:text-slate-200 dark:hover:bg-white/[.07]">{t("个人中心")}</Link>
            </div>
            <div className="grid gap-1">
              {MORE_LINKS.map((item) => (
                <Link key={item.to} to={item.to} onClick={() => setActiveMenu(null)} className="flex items-center gap-2.5 rounded-xl px-3 py-2 text-sm text-slate-600 transition hover:bg-slate-100 dark:text-slate-200 dark:hover:bg-white/[.07]">
                  <span className="w-4 text-center text-xs" aria-hidden="true">{item.icon}</span>{t(item.label)}
                </Link>
              ))}
            </div>
            <button type="button" onClick={() => setTheme(theme === "dark" ? "light" : "dark")} className="flex w-full items-center gap-2.5 rounded-xl px-3 py-2 text-left text-sm text-slate-600 dark:text-slate-200 sm:hidden">
              <span aria-hidden="true">{theme === "dark" ? "☀️" : "🌙"}</span>{theme === "dark" ? t("切换为浅色模式") : t("切换为深色模式")}
            </button>
            ...
            <button type="button" onClick={handleLogout} className="flex w-full items-center gap-2.5 rounded-xl px-3 py-2 text-left text-sm text-rose-500 transition hover:bg-rose-50 dark:hover:bg-rose-500/10">
              <ThemeIcon name="logout" size={15} />{t("退出认证")}{" "}</button>
          </div>
        </details>
        ...
        <button
          ...
          className="hidden h-10 w-10 shrink-0 place-items-center rounded-full border border-black/[.06] bg-white/75 text-[#4a5270] sm:grid transition hover:bg-white hover:text-[#0071e3] dark:border-white/10 dark:bg-white/[.06] dark:text-slate-200"
          aria-label={theme === "dark" ? t("切换为浅色模式") : t("切换为深色模式")}
