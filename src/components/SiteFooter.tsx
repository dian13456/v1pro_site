...
export function SiteFooter() {
  ...
  return (
    <footer className="mt-10 border-t border-white/20 pt-6 text-center text-xs text-slate-500 dark:text-slate-400">
      <p>{t("本站内容受保护，禁止未经授权的爬取、抓取与批量下载。")}<Link to="/terms" className="ml-1 text-violet-600 underline-offset-2 hover:underline dark:text-violet-300">{t("查看", "View ")}{t(TERMS_TITLE)}
        </Link>
      </p>
      <p className="mt-2">{t("传输到设备需")}<a
      ...
      <p className="mt-3">
        <a
          ...
          aria-label={t("前往工信部备案官网查询粤ICP备2026121845号")}
        >{t("粤ICP备2026121845号")}</a>
      </p>
    </footer>
  );
}
