...
export class AppErrorBoundary extends Component<Props, State> {
  ...
  render(): ReactNode {
    ...
    return <PageLoadError onReload={this.reload} />;
  }
}

export function PageLoadError({ onReload }: { onReload: () => void }) {
  const { t } = useI18n();
  return (
      <main className="flex min-h-screen items-center justify-center bg-[#f5f7fb] p-6 text-[#20253a]">
        <section className="w-full max-w-md rounded-2xl border border-[#dfe4ef] bg-white p-8 text-center shadow-lg">
          <LanguageSwitcher />
          <h1 className="mt-4 text-xl font-bold">{t("页面加载失败")}</h1>
          <p className="mt-3 text-sm leading-6 text-[#667085]">{t("可能是网络波动或网站刚完成更新，请重新加载。")}</p>
          <button
            type="button"
            onClick={onReload}
