...
function validatePhysicalShipping(shipping: MallShippingInput): string {
  if (
    ...
    return t("请完整填写收货人、手机、QQ、省市和详细地址");
  }
  if (!PHONE_PATTERN.test(shipping.phone.trim())) return t("手机号格式不正确");
  if (!QQ_PATTERN.test(shipping.qq.trim())) return t("QQ 号格式不正确");
  return "";
}
...
export default function ShopPage() {
  ...
  const handleRedeem = async (item: ShopItem) => {
    ...
    try {
      ...
      setNotice(result.message || t("已兑换「{v0}」", undefined, { v0: item.title }));
      setConfirmItem(null);
  ...
  const categoryOptions: Array<{ value: ShopCategory; label: string; icon: string }> = [
    { value: "all", label: t("全部商品"), icon: "▰" },
    { value: "grant_code", label: t("兑换码商品"), icon: "⌘" },
    { value: "reset_ai_share", label: t("次数权益"), icon: "↻" },
    { value: "add_credits", label: t("积分权益"), icon: "✦" },
    { value: "physical", label: t("实物商品"), icon: "⬡" },
  ];
  ...
  return (
    <div className="site-page-shell resource-library-shell min-h-screen text-[#2b3245]">
      <ResourceLibraryHeader keyword={keyword} onSearch={setKeyword} searchPlaceholder={t("搜索积分商品…")} />
        ...
        <div className="grid gap-5 lg:grid-cols-[218px_minmax(0,1fr)]">
          <aside className="space-y-[14px]">
            <section className="overflow-hidden rounded-[14px] border border-[#e6e9f2] bg-white dark:border-slate-800 dark:bg-slate-900">
              <h2 className="px-[18px] pb-2.5 pt-[18px] text-xs font-normal tracking-[1px] text-[#8a93a8] dark:text-slate-400">{t("商品分类")}</h2>
            ...
            <section className="rounded-[14px] border border-[#e6e9f2] bg-white p-[18px] dark:border-slate-800 dark:bg-slate-900">
              ...
              <Link to="/leaderboard" className="mt-3 flex items-center justify-between rounded-lg px-1 py-1.5 text-[13px] text-[#4a5270] transition hover:text-[#ff8a5c] dark:text-slate-300">
                <span>{t("🏆 查看积分榜")}</span><span>›</span>
              </Link>
            </section>

            <section className="rounded-[14px] border border-[#e6e9f2] bg-white p-[18px] dark:border-slate-800 dark:bg-slate-900">
              <p className="text-xs tracking-[1px] text-[#8a93a8] dark:text-slate-400">{t("快捷入口")}</p>
              <div className="mt-3 space-y-1">
                <Link to="/share" className="block rounded-lg px-2 py-2 text-[13px] text-[#4a5270] transition hover:bg-[#f6f7fd] hover:text-[#ff8a5c] dark:text-slate-300 dark:hover:bg-slate-800">{t("⬆ 上传素材赚积分")}</Link>
                <Link to="/" className="block rounded-lg px-2 py-2 text-[13px] text-[#4a5270] transition hover:bg-[#f6f7fd] hover:text-[#ff8a5c] dark:text-slate-300 dark:hover:bg-slate-800">{t("♡ 去素材中心互动")}</Link>
                <Link to="/mall" className="block rounded-lg px-2 py-2 text-[13px] text-[#4a5270] transition hover:bg-[#f6f7fd] hover:text-[#ff8a5c] dark:text-slate-300 dark:hover:bg-slate-800">{t("▦ 查看实物订单")}</Link>
              </div>
            </section>
          </aside>
          ...
          <div className="min-w-0 space-y-5">
            <section className="relative overflow-hidden rounded-2xl border border-[#e6e9f2] bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-7">
              ...
              <div className="relative grid gap-5 sm:grid-cols-[minmax(0,1fr)_220px] sm:items-center">
                <div>
                  <span className="inline-flex rounded-full bg-[#fff1eb] px-3 py-1 text-xs font-semibold text-[#ff7448] dark:bg-orange-500/10">{t("佳点会员积分")}</span>
                  <h1 className="mt-3 text-2xl font-extrabold tracking-tight text-[#2b3245] dark:text-white sm:text-3xl">{t("积分商城")}</h1>
                  <p className="mt-2 max-w-2xl text-sm leading-6 text-[#697086] dark:text-slate-300">{t("分享优质素材、参与互动即可获得积分。兑换由服务器实时扣除积分，成功后请及时保存权益信息。")}</p>
                  <div className="mt-4 flex flex-wrap gap-2">
                    <Link to="/share" className="rounded-full bg-gradient-to-br from-[#ff8a5c] to-[#ff6f9c] px-4 py-2 text-sm font-semibold text-white shadow-[0_4px_12px_rgba(255,138,92,.28)] transition hover:-translate-y-0.5">{t("上传素材赚积分")}</Link>
                    <Link to="/" className="rounded-full border border-[#e6e9f2] bg-white px-4 py-2 text-sm font-medium text-[#4a5270] transition hover:border-[#ff8a5c] hover:text-[#ff8a5c] dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200">{t("返回素材主页")}</Link>
                  </div>
                </div>
                <div className="rounded-2xl border border-[#edf0f6] bg-[#fafbfe] p-5 text-center dark:border-slate-700 dark:bg-slate-950/50">
                  <p className="text-xs text-[#8a93a8] dark:text-slate-400">{t("当前可用积分")}</p>
                  <p className="mt-1 text-4xl font-extrabold tracking-tight text-[#ff7448]">{loading ? "—" : formatCredits(credits)}</p>
                  <p className="mt-2 text-[11px] text-[#9aa2b5] dark:text-slate-500">{t("兑换时实时校验余额")}</p>
                </div>
              </div>
            </section>
              ...
              <section className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5 dark:border-emerald-500/30 dark:bg-emerald-500/10">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div className="min-w-0 flex-1">
                    <p className="font-semibold text-emerald-800 dark:text-emerald-200">{t("兑换成功")}</p>
                    <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">{t("兑换码仅在本次结果中展示，请立即复制保存。")}</p>
                  ...
                  <button type="button" onClick={() => void handleCopyCode()} className="rounded-full bg-emerald-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-emerald-500">{copied ? t("已复制") : t("复制兑换码")}</button>
                </div>
              </section>
            ) : null}
            ...
            {redeemOrderId ? (
              <section className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5 dark:border-emerald-500/30 dark:bg-emerald-500/10">
                <p className="font-semibold text-emerald-800 dark:text-emerald-200">{t("实物兑换成功，等待发货")}</p>
                <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">{t("订单号：")}<span className="select-all font-mono font-semibold">{redeemOrderId}</span></p>
                <div className="mt-4 flex flex-wrap gap-2">
                  <Link to="/mall" className="rounded-full bg-emerald-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-emerald-500">{t("查看实物商城订单")}</Link>
                  <span className="self-center text-xs text-slate-500 dark:text-slate-400">{t("进入后点击“我的订单”查看物流单号。")}</span>
                </div>
              </section>
            ) : null}
            ...
            <section aria-labelledby="shop-products-title">
              <div className="mb-3 flex flex-wrap items-end justify-between gap-3">
                <div>
                  <h2 id="shop-products-title" className="text-lg font-extrabold text-[#2b3245] dark:text-white">{t("可兑换商品")}</h2>
                  <p className="mt-1 text-xs text-[#8a93a8] dark:text-slate-400">{t("选择商品后确认兑换，积分不足会显示所差积分。")}</p>
                </div>
                <span className="rounded-full border border-[#e6e9f2] bg-white px-3 py-1.5 text-xs text-[#697086] dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300">{t("共")}{" "}{visibleItems.length}{" "}{t("件")}</span>
              </div>
              <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                {loading ? <div className="col-span-full rounded-2xl border border-[#e6e9f2] bg-white py-16 text-center text-sm text-[#8a93a8] dark:border-slate-800 dark:bg-slate-900">{t("加载商品中…")}</div> : null}
                {!loading && visibleItems.length === 0 ? <div className="col-span-full rounded-2xl border border-dashed border-[#cfd5ea] bg-white py-16 text-center text-sm text-[#8a93a8] dark:border-slate-700 dark:bg-slate-900">{t("没有找到符合条件的商品。")}</div> : null}
            ...
            return (
              <article key={item.id} className="group flex min-h-full flex-col overflow-hidden rounded-2xl border border-[#e6e9f2] bg-white shadow-sm transition duration-200 hover:-translate-y-1 hover:shadow-xl dark:border-slate-800 dark:bg-slate-900">
                <div className={`relative grid aspect-[1.618] place-items-center bg-gradient-to-br ${cardGradients[itemIndex % cardGradients.length]}`}>
                  ...
                  {item.effect.type === "physical" && typeof item.stock === "number" ? <span className="absolute bottom-3 right-3 rounded-full bg-slate-900/55 px-2.5 py-1 text-[11px] font-medium text-white">{t("库存")}{" "}{Math.max(0, item.stock)}</span> : null}
                </div>
                <div className="flex flex-1 flex-col p-3.5">
                  ...
                  {item.effect.type === "physical" && typeof item.stock === "number" ? (
                    <p className={`mt-2 text-xs font-medium ${soldOut ? "text-rose-600 dark:text-rose-300" : "text-emerald-600 dark:text-emerald-300"}`}>
                      {soldOut ? t("当前库存不足") : t("剩余库存 {v0} 件", undefined, { v0: item.stock })}
                    </p>
                  ) : null}
                  <div className="mt-auto pt-4">
                    <div className="mb-2 flex items-center justify-between text-[11px]">
                      <span className="text-[#9aa2b5] dark:text-slate-500">{t("兑换进度")}</span>
                      <span className={affordable ? "font-medium text-emerald-600 dark:text-emerald-300" : "text-[#8a93a8] dark:text-slate-400"}>{affordable ? t("积分已满足") : t("还差 {v0} 积分", undefined, { v0: formatCredits(missingCredits) })}</span>
                    </div>
                    ...
                    <div className="mt-3 flex items-center justify-between gap-3 border-t border-[#edf0f6] pt-3 dark:border-slate-800">
                      <div><span className="text-xl font-extrabold text-[#ff7448]">{formatCredits(item.cost)}</span><span className="ml-1 text-[11px] text-[#8a93a8] dark:text-slate-400">{t("积分")}</span></div>
                      <button
                        type="button"
                      ...
                      >
                        {soldOut ? t("库存不足") : affordable ? t("立即兑换") : t("积分不足")}
                      </button>
                    </div>
                  </div>
                </div>
            ...
            <section className="rounded-2xl border border-[#e6e9f2] bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-6">
              <div className="mb-4">
                <h2 className="text-lg font-extrabold text-[#2b3245] dark:text-white">{t("积分怎么获得")}</h2>
                <p className="mt-1 text-xs text-[#8a93a8] dark:text-slate-400">{t("积分自动计入当前设备账号，无需手动领取。")}</p>
              </div>
              <div className="grid gap-3 sm:grid-cols-3">
                <div className="rounded-xl bg-[#fff3eb] p-4 dark:bg-orange-500/10">
                  <div className="text-sm font-bold text-[#2b3245] dark:text-slate-100">{t("素材被点赞")}</div>
                  <p className="mt-1 text-xs leading-5 text-[#697086] dark:text-slate-400">{t("每次有效点赞 +{credits} 积分，不受点赞者每日次数限制。", undefined, { credits: formatCredits(likeRewardCredits) })}</p>
                </div>
                <div className="rounded-xl bg-[#fff0f5] p-4 dark:bg-pink-500/10">
                  <div className="text-sm font-bold text-[#2b3245] dark:text-slate-100">{t("点赞优质素材")}</div>
                  <p className="mt-1 text-xs leading-5 text-[#697086] dark:text-slate-400">{t("每天前 {count} 次有效，每次 +{credits}，每日最高 {cap} 积分。", undefined, { count: actorLikeDailyLimit, credits: formatCredits(actorLikeRewardCredits), cap: formatCredits(actorLikeDailyCapCredits) })}</p>
                </div>
                <div className="rounded-xl bg-[#eef9f8] p-4 dark:bg-cyan-500/10">
                  <div className="text-sm font-bold text-[#2b3245] dark:text-slate-100">{t("素材被下载")}</div>
                  <p className="mt-1 text-xs leading-5 text-[#697086] dark:text-slate-400">{t("每次有效下载 +{credits} 积分，每日最高 {cap} 积分。", undefined, { credits: formatCredits(downloadRewardCredits), cap: formatCredits(downloadDailyCapCredits) })}</p>
                </div>
              </div>
              <p className="mt-3 text-xs text-[#8a93a8] dark:text-slate-400">{t("不能给自己点赞，也不能通过下载自己的素材获得积分。")}</p>
            </section>

            <section className="rounded-2xl border border-[#e6e9f2] bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-6">
              <h2 className="text-lg font-extrabold text-[#2b3245] dark:text-white">{t("积分记录")}</h2>
              <p className="mt-1 text-xs text-[#8a93a8] dark:text-slate-400">{t("最近的积分收入和兑换支出。")}</p>
      ...
      {confirmItem ? (
        <div className="fixed inset-0 z-[100] flex items-start justify-center overflow-y-auto bg-[rgba(30,35,55,.48)] p-4 backdrop-blur-[3px] sm:items-center" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget && !redeemingId) setConfirmItem(null); }}>
          <div role="dialog" aria-modal="true" aria-labelledby="redeem-confirm-title" className="my-auto max-h-[calc(100dvh-2rem)] w-full max-w-2xl overflow-y-auto rounded-[18px] border border-[#e6e9f2] bg-white p-4 shadow-[0_24px_60px_rgba(35,40,60,.28)] dark:border-slate-700 dark:bg-slate-900 sm:p-6">
            ...
            <h2 id="redeem-confirm-title" className="mt-4 text-xl font-bold text-slate-900 dark:text-white">{t("确认兑换")}</h2>
            <p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">{t("确定使用")}{" "}<strong className="text-[#ff7448]">{formatCredits(confirmItem.cost)}{" "}{t("积分")}</strong>{t("兑换“")}{confirmItem.title}{t("”吗？兑换成功后积分将立即扣除。")}</p>
            {confirmItem.effect.type === "physical" ? (
              <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50/80 p-4 dark:border-slate-700 dark:bg-slate-950/50">
                <div className="mb-3">
                  <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-100">{t("收货信息")}</h3>
                  <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">{t("地址仅用于本次发货，并会加密保存在服务器。")}</p>
                </div>
                <div className="grid gap-3 sm:grid-cols-2">
                  <SiteInput placeholder={t("收货人姓名 *")} value={shipping.name} onChange={(event) => updateShipping("name", event.target.value)} />
                  <SiteInput inputMode="tel" placeholder={t("手机号 *")} value={shipping.phone} onChange={(event) => updateShipping("phone", event.target.value)} />
                  <SiteInput inputMode="numeric" placeholder={t("QQ 号 *")} value={shipping.qq} onChange={(event) => updateShipping("qq", event.target.value)} />
                  <SiteInput placeholder={t("微信号（选填）")} value={shipping.wechat || ""} onChange={(event) => updateShipping("wechat", event.target.value)} />
                  <SiteInput placeholder={t("省份 *")} value={shipping.province} onChange={(event) => updateShipping("province", event.target.value)} />
                  <SiteInput placeholder={t("城市 *")} value={shipping.city} onChange={(event) => updateShipping("city", event.target.value)} />
                </div>
                <SiteTextarea className="mt-3" rows={2} placeholder={t("详细收货地址 *")} value={shipping.address} onChange={(event) => updateShipping("address", event.target.value)} />
                <SiteTextarea className="mt-3" rows={2} placeholder={t("订单备注（选填）")} value={shipping.remark || ""} onChange={(event) => updateShipping("remark", event.target.value)} />
              </div>
            ) : null}
