...
function formatCredits(value: number): string {
  return formatNumber(Math.max(0, value || 0), { maximumFractionDigits: 2 });
}
...
function Avatar({ entry, large = false }: { entry: CreditLeaderboardEntry; large?: boolean }) {
  ...
  if (entry.avatarUrl) {
    return <img src={entry.avatarUrl} alt={t("{v0}的头像", undefined, {v0: entry.displayName})} className={`${size} rounded-full border-4 border-white object-cover shadow-md`} />;
  }
  return (
    <div className={`${size} grid shrink-0 place-items-center rounded-full border-4 border-white bg-gradient-to-br from-[#ff9d72] to-[#7c6cf0] font-extrabold text-white shadow-md`}>
      {entry.displayName.trim().slice(0, 1) || t("佳")}
    </div>
  );
}
...
function CreatorNameLink({ entry, className = "" }: { entry: CreditLeaderboardEntry; className?: string }) {
  ...
  return (
    <Link
      ...
      title={t("查看 {v0} 上传的素材", undefined, {v0: entry.displayName})}
    >
      {entry.displayName}
    </Link>
  );
...
export default function CreditLeaderboardPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(formatClientError(err, t("积分榜加载失败，请稍后重试")));
    } finally {
      setLoading(false);
    }
  }, []);
      ...
      <main className="mx-auto max-w-[1120px] px-4 py-6 sm:px-6">
          ...
          <div className="relative flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <h1 className="mt-1 text-2xl font-extrabold sm:text-[28px]">{t("积分排行榜")}</h1>
                <p className="mt-1 text-sm text-[#8a93a8]">{t("积分越多，排名越靠前 · 同积分共享名次")}</p>
              </div>
            </div>
            <div className="flex gap-3">
              <div className="min-w-[104px] rounded-[16px] bg-[#fff7f2] px-4 py-3 text-center">
                <p className="text-[11px] font-semibold text-[#8a93a8]">{t("参与用户")}</p>
        ...
        {current ? (
          <section className="mt-[14px] flex flex-col gap-3 rounded-[18px] bg-gradient-to-r from-[#7468ee] to-[#598bff] px-5 py-4 text-white shadow-[0_10px_26px_rgba(89,139,255,.2)] sm:flex-row sm:items-center sm:justify-between sm:px-7">
            <div>
              <p className="text-xs font-semibold text-white/70">{t("我的当前排名")}</p>
              <div className="mt-1 flex items-baseline gap-3">
                <span className="text-3xl font-extrabold">{t("第 {rank} 名", "Rank {rank}", { rank: formatNumber(current.rank) })}</span>
                {!currentInList ? <span className="rounded-full bg-white/15 px-2.5 py-1 text-xs">{t("未进入前 50 名")}</span> : null}
              </div>
            </div>
            <div className="flex items-center gap-3 sm:text-right">
              <div>
                <p className="text-sm font-bold">{current.displayName}</p>
                <p className="mt-0.5 text-lg font-extrabold">{formatCredits(current.credits)} <span className="text-xs font-semibold text-white/75">{t("积分")}</span></p>
              </div>
        ...
        {error ? (
          <section className="mt-[14px] rounded-[18px] border border-[#ffd7d1] bg-[#fff7f5] px-6 py-10 text-center">
            <p className="font-semibold text-[#d85d54]">{t(error)}</p>
            <button type="button" onClick={() => void load()} className="mt-4 rounded-full bg-[#ff7d67] px-5 py-2 text-sm font-bold text-white">{t("重新加载")}</button>
          </section>
        ) : loading ? (
          <section className="mt-[14px] grid min-h-[340px] place-items-center rounded-[18px] border border-[#e6e9f2] bg-white">
            <div className="text-center"><div className="mx-auto h-9 w-9 animate-spin rounded-full border-4 border-[#f0eefe] border-t-[#7c6cf0]" /><p className="mt-3 text-sm text-[#8a93a8]">{t("正在统计积分排名…")}</p></div>
          </section>
        ) : entries.length === 0 ? (
          <section className="mt-[14px] rounded-[18px] border border-[#e6e9f2] bg-white px-6 py-16 text-center text-[#8a93a8]">{t("暂时还没有积分排名数据")}</section>
        ) : (
          <>
              ...
              {podiumEntries.map((entry) => {
                ...
                return (
                  <article key={`${entry.rank}-${entry.displayName}-${index}`} className={`relative overflow-hidden rounded-[20px] border border-white bg-gradient-to-br ${meta.gradient} px-5 py-7 text-center shadow-[0_10px_28px_rgba(43,50,69,.08)] ${index === 0 ? "md:-translate-y-3 md:pb-9 md:pt-8" : ""} ${entry.isCurrent ? "ring-2 ring-[#7c6cf0] ring-offset-2" : ""}`}>
                    <span className="absolute left-4 top-4 rounded-full bg-white/70 px-2.5 py-1 text-[11px] font-bold text-[#697086]">{t("第 {rank} 名", "Rank {rank}", { rank: formatNumber(entry.rank) })}</span>
                    {entry.isCurrent ? <span className="absolute right-4 top-4 rounded-full bg-[#7c6cf0] px-2.5 py-1 text-[11px] font-bold text-white">{t("我")}</span> : null}
                    ...
                    <p className={`mt-1 text-[11px] font-bold uppercase tracking-[.18em] ${meta.color}`}>{t(meta.label)}</p>
                    <div className="mt-4 rounded-[14px] bg-white/65 px-3 py-2.5">
                      <strong className="text-2xl">{formatCredits(entry.credits)}</strong><span className="ml-1 text-xs font-semibold text-[#8a93a8]">{t("积分")}</span>
                    </div>
                  </article>
                );
              })}
            ...
            {remaining.length > 0 ? (
              <section className="mt-[14px] overflow-hidden rounded-[18px] border border-[#e6e9f2] bg-white shadow-[0_10px_28px_rgba(43,50,69,.05)]">
                <div className="flex items-center justify-between border-b border-[#eef0f6] px-5 py-4 sm:px-7">
                  <div><h2 className="font-extrabold">{t("排行榜")}</h2><p className="mt-0.5 text-xs text-[#8a93a8]">{t("展示积分排名前 50 名")}</p></div>
                  {updatedAt ? <span className="text-[11px] text-[#a0a7b8]">{t("更新于")}{" "}{formatDate(updatedAt)}</span> : null}
                </div>
                <div className="divide-y divide-[#f0f1f6]">
                  {remaining.map((entry, index) => (
                    <div key={`${entry.rank}-${entry.displayName}-${index + 3}`} className={`grid grid-cols-[46px_44px_minmax(0,1fr)_auto] items-center gap-3 px-5 py-3.5 sm:px-7 ${entry.isCurrent ? "bg-[#f3f1ff]" : "transition hover:bg-[#fafbfe]"}`}>
                      ...
                      <Avatar entry={entry} />
                      <div className="min-w-0"><p className="truncate text-sm font-bold"><CreatorNameLink entry={entry} /></p>{entry.isCurrent ? <p className="mt-0.5 text-[11px] font-semibold text-[#7c6cf0]">{t("我的排名")}</p> : null}</div>
                      <div className="text-right"><strong className="text-base text-[#ff7d67] sm:text-lg">{formatCredits(entry.credits)}</strong><span className="ml-1 text-[11px] text-[#8a93a8]">{t("积分")}</span></div>
                    </div>
                  ))}
                </div>
              </section>
