  ...
  return (
    ...
    >
      <SitePanel className="max-w-md text-center">
        ...
        <h1 className="mt-2 text-4xl font-semibold text-slate-900 dark:text-slate-50">{t("页面不存在")}</h1>
        <p className="mt-3 text-sm text-slate-600 dark:text-slate-300">{t("你访问的地址可能已失效或输入有误。")}</p>
        <Link to="/" className="mt-6 inline-block">
          <SiteButton type="button">{t("返回素材中心")}</SiteButton>
        </Link>
      </SitePanel>
    </SitePageLayout>
  );
