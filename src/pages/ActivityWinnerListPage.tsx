    ...
    >
      <SitePanel accent className="overflow-hidden p-0">
        <div className="bg-gradient-to-r from-amber-500/95 via-orange-500/90 to-rose-500/85 px-6 py-6 text-white">
          ...
          <h1 className="mt-2 text-2xl font-semibold md:text-3xl">{t("中奖名单公示")}</h1>
          <p className="mt-2 max-w-2xl text-sm leading-7 text-white/90">
            {(data?.activityTitle ? t(data.activityTitle) : "") || t("设备用户专属抽奖活动")}{" "}{t("· 每日自动开奖后更新")}{" "}</p>
        </div>
      </SitePanel>

      <SiteAlert variant="info">{t("公示名单展示用户昵称、脱敏后的设备 SN、开奖期次与中奖时间；姓名、手机号、收货地址等个人信息不会公开。")}{" "}</SiteAlert>

      {loading ? <SiteLoadingBlock>{t("加载中奖名单…")}</SiteLoadingBlock> : null}
      ...
      {!loading && data ? (
        <SitePanel>
          <SiteSectionTitle
            ...
            description={
              data.prizeTitle
                ? t("当前奖品：{v0}。共 {v1} 条记录。", undefined, { v0: t(data.prizeTitle), v1: data.winners.length })
                : t("共 {v0} 条记录。", undefined, { v0: data.winners.length })
            }
          />
          {data.winners.length === 0 ? (
            <p className="mt-4 text-sm text-slate-600 dark:text-slate-300">{t("暂无中奖记录，敬请期待首期开奖。")}</p>
          ) : (
            <div className="mt-4 overflow-x-auto rounded-2xl border border-white/25 dark:border-white/10">
              <table className="min-w-full text-left text-sm">
                <thead className="bg-white/60 text-xs uppercase tracking-wide text-slate-500 dark:bg-slate-900/50 dark:text-slate-400">
                  <tr>
                    <th className="px-4 py-3 font-medium">{t("开奖期次")}</th>
                    <th className="px-4 py-3 font-medium">{t("用户昵称")}</th>
                    <th className="px-4 py-3 font-medium">{t("中奖 SN（脱敏）")}</th>
                    <th className="px-4 py-3 font-medium">{t("奖品")}</th>
                    <th className="px-4 py-3 font-medium">{t("中奖时间")}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/20 dark:divide-white/10">
                  {data.winners.map((winner, index) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-slate-700 dark:text-slate-200">{t(winner.prizeTitle || data.prizeTitle)}</td>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <Link to="/activities">
          <SiteButton type="button" className="bg-transparent text-slate-700 ring-1 ring-slate-300 dark:text-slate-200 dark:ring-slate-600">{t("活动中心")}{" "}</SiteButton>
        </Link>
      </div>
    </SitePageLayout>
  );
