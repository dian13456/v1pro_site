...
function ActivityBadge({ status }: { status: ActivityStatus }) {
  ...
  return (
    <span className={`rounded-full px-3 py-1 text-[11px] font-semibold ${STATUS_STYLES[status]}`}>
      {t(ACTIVITY_STATUS_LABELS[status])}
    </span>
  );
}
    ...
    <article className="flex h-full flex-col rounded-[16px] border border-[#e6e9f2] bg-white p-5 shadow-[0_8px_24px_rgba(43,50,69,.04)] transition hover:-translate-y-0.5 hover:shadow-[0_12px_30px_rgba(43,50,69,.08)]">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="rounded-full bg-[#f0edff] px-3 py-1 text-[11px] font-semibold text-[#7c6cf0]">
          {t(ACTIVITY_CATEGORY_LABELS[item.category])}
        </span>
      </div>

      <h3 className="mt-3 text-[17px] font-extrabold text-[#2b3245]">{t(item.title)}</h3>
      <p className="mt-2 text-[13px] leading-6 text-[#6f7890]">{t(item.summary)}</p>
      <p className="mt-3 flex-1 text-[12px] leading-6 text-[#8a93a8]">{t(item.body)}</p>
        ...
        {item.linkTo && item.linkLabel ? (
          <div className="flex flex-wrap gap-2">
            ...
            {item.id === "device-lottery" ? (
              <Link to="/activities/winners" className="rounded-full border border-[#e6e9f2] bg-white px-4 py-2 text-xs font-semibold text-[#4a5270] hover:border-[#7c6cf0] hover:text-[#7c6cf0]">{t("中奖名单")}{" "}</Link>
            ) : null}
          </div>
        ) : null}
      </div>
    ...
    <div className="site-page-shell resource-library-shell min-h-screen text-[#2b3245]">
      ...
      <main className="mx-auto max-w-[1120px] space-y-[14px] px-4 py-6 sm:px-6">
        <section className="overflow-hidden rounded-[18px] border border-[#e6e9f2] bg-white shadow-[0_10px_30px_rgba(43,50,69,.06)]">
          <div className="grid grid-cols-[48px_minmax(0,1fr)] sm:grid-cols-[64px_minmax(0,1fr)_auto] items-center gap-x-4 gap-y-3 px-5 py-6 sm:px-8">
            <div className="grid h-12 w-12 sm:h-16 sm:w-16 place-items-center rounded-[20px] bg-gradient-to-br from-[#ff8a5c] to-[#7c6cf0] text-3xl text-white shadow-[0_8px_20px_rgba(124,108,240,.24)]">🎁</div>
            <div className="min-w-0">
              <p className="text-[11px] font-bold uppercase tracking-[.2em] text-[#ff8a5c]">Events & Rewards</p>
              <h1 className="mt-1 text-2xl font-extrabold">{t("活动中心")}</h1>
            </div>
            <div className="col-span-2 rounded-[16px] sm:col-span-1 sm:col-start-3 sm:row-start-1 bg-[#fff7f2] px-4 py-3 text-center sm:row-span-2 sm:px-6">
              <p className="text-[11px] font-semibold text-[#8a93a8]">{t("正在进行")}</p>
              <p className="mt-0.5 text-2xl font-extrabold text-[#ff8a5c]">{ongoingCount}</p>
            </div>
            <p className="col-span-2 max-w-2xl text-[13px] leading-6 text-[#8a93a8] sm:col-span-1 sm:col-start-2">{t(ACTIVITY_CENTER_INTRO)}</p>
          </div>
        </section>
      ...
      {promoActivity ? (
        <section className="overflow-hidden rounded-[18px] border border-[#e6e9f2] bg-white shadow-[0_10px_30px_rgba(43,50,69,.05)]">
          <div className="bg-gradient-to-r from-[#ff8a5c] via-[#ff6f9c] to-[#7c6cf0] px-6 py-5 text-white sm:px-8">
            <p className="text-[11px] font-bold uppercase tracking-[.22em] text-white/80">{t("精选活动")}</p>
            <h2 className="mt-2 text-xl font-extrabold sm:text-2xl">{t(promoActivity.title)}</h2>
            <p className="mt-2 max-w-3xl text-[13px] leading-6 text-white/90">{t(promoActivity.summary)}</p>
          </div>
          <div className="space-y-4 p-6 sm:px-8">
            <div className="flex flex-wrap items-center gap-2">
              <ActivityBadge status={promoActivity.status} />
              <span className="rounded-full bg-[#f0edff] px-3 py-1 text-[11px] font-semibold text-[#7c6cf0]">
                {t(ACTIVITY_CATEGORY_LABELS[promoActivity.category])}
              </span>
              <span className="text-[11px] text-[#8a93a8]">{formatActivityRange(promoActivity)}</span>
            </div>
            <p className="text-[13px] leading-7 text-[#6f7890]">{t(promoActivity.body)}</p>
      ...
      <div className="flex items-end justify-between gap-3 px-1 pt-2">
        <div>
          <h2 className="text-lg font-extrabold">{t("全部活动")}</h2>
          <p className="mt-1 text-xs text-[#8a93a8]">{t("浏览当前可参与的活动与平台更新")}</p>
        </div>
      </div>
