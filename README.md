...
高端风格产品资源下载站，接入 WebUSB 设备认证（VID `0x0483` / PID `0x66AA`），并通过后端接口完成序列号鉴权与 COS 私有签名下载。
...
- 在仓库 Settings -> Secrets and variables -> Actions -> Variables 中设置 `VITE_API_BASE=https://你的后端域名`
- Actions 构建时使用该变量输出前端配置
...
- `COS_SECRET_ID`
- `COS_SECRET_KEY`
- `IMAGE_COS_BUCKET`（默认回退 `COS_BUCKET`）
- `IMAGE_COS_REGION`（默认回退 `COS_REGION`）
- `IMAGE_COS_SECRET_ID`（默认回退 `COS_SECRET_ID`）
- `IMAGE_COS_SECRET_KEY`（默认回退 `COS_SECRET_KEY`）
- `JWT_SECRET`
- `ALLOWED_VID`（默认 `0483`）
- `ALLOWED_PID`（默认 `66AA`）
