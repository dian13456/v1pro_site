...
func applyVideoModerationOutcome(
	store *ImageReviewStore,
	...
	case "REVIEW":
		if store == nil {
			return PendingImageReview{}, false, fmt.Errorf(
				"%w: %s",
				ErrImageModerationReview,
				formatModerationMessage(outcome, "视频需人工复核，暂无法上传"),
			)
		}
		input.Outcome = outcome
		item := EnqueueVideoReview(store, input)
		return item, true, nil
	default:
		return PendingImageReview{}, false, fmt.Errorf(
			"%w: %s",
			ErrImageModerationBlocked,
			formatModerationMessage(outcome, "视频未通过内容安全审核"),
		)
	}
}
